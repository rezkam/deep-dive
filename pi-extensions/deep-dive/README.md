...
![Deep Dive screenshot](assets/screenshot.png)
...
1. You run `/deep-dive` and the extension starts an HTTP + WebSocket server and spawns a pi subagent
2. The subagent reads files, follows imports, and builds understanding, streaming events back via JSON-RPC
3. The server bridges those events to the browser over WebSocket so you see the document build in real time
